...
- **Description**: The manifest pinned `blueprint-compiler` and
  `gst-plugins-rs` by tag alone. A git tag is a mutable ref in the source
  repository: whoever controls those accounts, or compromises the
  server, changes what gets compiled without the manifest changing.
- **Changes**:
  - `commit` added next to each `tag`. Resolved with
    `git ls-remote <url> refs/tags/<tag>` and verified against the
    remotes: `be7beddc…` for blueprint-compiler v0.16.0,
    `4281db13…` for gst-plugins-rs 0.13.5. flatpak-builder fails the
    build when the tag no longer resolves to the pinned commit.
  - Two `x-comment-*` blocks recording the rule for future bumps and the
    open offline-build work. `x-` keys are ignored by flatpak-builder,
    which is the only way to leave a note in a JSON manifest.
- **Deliberately not done here**: removing `--share=network`. That needs
  `cargo-sources.json` generated by `flatpak-cargo-generator.py`, which
  is a large generated file whose correctness only shows up by building
  the Flatpak end to end. Bundling it with the commit pins would mean
  shipping an unverified change alongside a verified one.
- **Acceptance criteria**:
  - Every git source carries a full `commit`. **DONE**.
  - Each commit verified to be what its tag currently resolves to.
    **DONE** (2026-08-06).
  - Manifest is valid JSON. **DONE**.
  - **CI validation PENDING**: the `flatpak` job builds the manifest on
